...
# Optional: shared explicit model override for ordinary Codex Generation and Codex Agent.
# CODEX_MODEL=gpt-5.5
...
- `GENERATION_BACKEND=litellm|codex_app_server|codex_cli|claude_code_cli|opencode_cli`. `codex_app_server` is the official Codex App Server ordinary-Generation adapter. Local CLI backends are generation backends, not LiteLLM providers; do not set `LITELLM_MODEL=codex_cli/...`, `LITELLM_MODEL=claude_code_cli/...`, or `LITELLM_MODEL=opencode_cli/...`.
- With `GENERATION_BACKEND=codex_app_server`, regular reports, market review, screening ranking, and registered lightweight text/JSON paths use the Codex sign-in state. `AGENT_BACKEND=codex_app_server` remains the separate switch for ask-stock Agent/Paper tool sessions. Enable both by setting both backend variables; there is no third global switch. `CODEX_MODEL` is an optional explicit model override; empty means the Codex runtime default.
...
- For legacy generation backends, an unset `GENERATION_FALLBACK_BACKEND` defaults to `litellm`; when the primary is `codex_app_server`, Config defaults to fail closed. For either primary, an explicit empty value in local `.env` disables backend-level fallback, and a fallback equal to the primary is treated as no-op. The bundled GitHub Actions workflow explicitly exports `litellm` when this variable is not configured; to disable backend fallback there, set the fallback to the primary backend, for example `GENERATION_BACKEND=codex_cli` + `GENERATION_FALLBACK_BACKEND=codex_cli`.
...
- The Web generation-backend quick check performs only configuration, Codex executable/protocol, account, and rate-limit RPC checks. It does not write config, reload runtime, access market data, send a model turn, or consume generation quota. The unified projection includes generation primary/fallback, Agent primary, fallback policy, Codex model/platform/binary, safe account state, and the first-phase Vision/Deep Research exceptions.
- `GET /api/v1/system/config/generation-backends/status` returns the saved combination status. Unsaved drafts use `POST /api/v1/system/config/generation-backends/status/preview`; use `POST /api/v1/system/config/generation-backends/quick-check` when an explicit account/rate-limit check is needed. Masked secrets preserve saved values. `health_status` and `last_error_code/message` describe only the current computation, not persisted historical health.
- Text/JSON smoke is a separate explicit administrator operation. It uses a fixed prompt and zero-tool ephemeral session, never persists a business report or accesses market data. Codex smoke returns a subscription-quota warning before execution by default; send `confirm_quota_risk=true` to authorize one real model request. The quick-check and smoke POST APIs require a valid admin session and a same-origin Origin/Referer (or browser `X-Requested-With`) marker.

### Codex Local Agent (Phase 6 Experimental Prototype)

`AGENT_BACKEND` selects only the runtime for ask-stock Chat/Paper tool sessions; it does not by itself change regular reports. Regular reports use Codex only when `GENERATION_BACKEND=codex_app_server`; LiteLLM Multi Agent and Deep Research remain separate capabilities:
...
- Codex Agent now receives the profile-bound `portfolio_readonly` ToolSurface: live quotes, historical bars, chip/capital-flow data, technical recalculation, news and comprehensive intelligence, market indices/sectors, fundamentals/announcements/research reports, overall/strategy/per-stock backtest details, and read-only Paper/portfolio/risk snapshots. Descriptors and execution are both filtered by the current Execution Profile, stock scope, and cancellation-safety contract. Paper proposal/approval, real-trading writes, Shell, files, MCP, and plugins remain unavailable. Empty data or an upstream failure must be reported as missing data, not as missing Agent configuration. An explicit symbol or one uniquely selected Web stock establishes stock scope; DSA does not guess across ambiguous listings.
